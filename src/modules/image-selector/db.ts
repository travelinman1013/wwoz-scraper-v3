...
export class ImageDb {
  ...
  upsertImage(row: Partial<ImageRow> & { path: string; mtimeMs: number }): void {
  ...
  getRow(p: string): ImageRow | null {
    const stmt = this.db.prepare(`SELECT * FROM images WHERE path = ?`);
    return (stmt.get(p) as ImageRow) ?? null;
  }

  getUsedDhashes(): string[] {
    const stmt = this.db.prepare(`SELECT dhash FROM images WHERE used = 1 AND dhash IS NOT NULL`);
    return (stmt.all() as { dhash: string }[]).map((r) => r.dhash);
  }

  getCandidates(minSharpness: number, minBrightness: number): ImageRow[] {
    const stmt = this.db.prepare(`
      SELECT * FROM images
      WHERE used = 0
    ...
    `);
    return stmt.all({ minSharpness, minBrightness }) as ImageRow[];
  }

  all(): ImageRow[] {
    return this.db.prepare('SELECT * FROM images').all() as ImageRow[];
  }
