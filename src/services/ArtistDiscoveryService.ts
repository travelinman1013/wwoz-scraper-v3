...
import type { ProcessedArchivesDB, ProcessedArchiveState } from '../types/index.js';
...
export class ArtistDiscoveryService {
   ...
   */
  async processArchive(archivePath: string): Promise<void> {
    ...
    if (!config.artistDiscovery.forceReprocess && this.wasProcessedSuccessfully(archiveDate)) {
      ...
      Logger.debug(
        `Archive ${archiveDate} already processed successfully at ${state.processedAt}; skipping.`
      );
      return;
    }
    ...
    try {
      ...
      const state: ProcessedArchiveState = {
   ...
   */
  private async executePythonScript(archivePath: string, archiveDate: string): Promise<void> {
   ...
   */
  private extractDateFromPath(archivePath: string): string | null {
   ...
   */
  private wasProcessedSuccessfully(archiveDate: string): boolean {
    const state = this.stateDb[archiveDate];
   ...
   */
  private saveState(archiveDate: string, state: ProcessedArchiveState): void {
