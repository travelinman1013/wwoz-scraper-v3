...
export async function openSharp(filePath) {
    try {
...
export async function loadJpegBuffer(filePath) {
    try {
...
export async function readImageInfo(filePath) {
    try {
        const s = await openSharp(filePath);
        if (s) {
        ...
        try {
            ...
            const width = Number(tags.ImageWidth ?? tags.ExifImageWidth ?? tags.PreviewImageWidth ?? tags.SourceImageWidth ?? null);
            const height = Number(tags.ImageHeight ?? tags.ExifImageHeight ?? tags.PreviewImageHeight ?? tags.SourceImageHeight ?? null);
...
export async function computeSharpnessAndBrightness(filePath) {
    ...
    try {
        ...
        const lap = new Array(w * h).fill(0);
        for (let y = 1; y < h - 1; y++) {
            for (let x = 1; x < w - 1; x++) {
                const i = y * w + x;
                const center = pix[i];
