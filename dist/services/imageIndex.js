...
import { ImageDb } from '../modules/image-selector/db.js';
...
async function walkFolder(dir) {
    ...
    while (stack.length) {
        ...
        for (const ent of entries) {
            ...
            if (ent.isDirectory())
                stack.push(p);
            else if (ent.isFile() && isImageFile(p))
                files.push(p);
        }
    }
    return files;
}
async function indexOne(db, p) {
...
export async function indexAllImages() {
    ...
    try {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            try {
                ...
                updated++;
                if (updated % 200 === 0)
                    Logger.info(`Indexed ${updated}/${files.length}...`);
            }
            catch (err) {
        ...
        const all = db.all();
        const toScore = all.filter((r) => r.used === 0 &&
        ...
        for (let i = 0; i < toScore.length; i++) {
            ...
            try {
                ...
                if (s !== null && s !== undefined) {
                    ...
                    scored++;
                    if (scored % 100 === 0)
                        Logger.info(`Scored ${scored}/${toScore.length}...`);
                }
            }
            catch {
